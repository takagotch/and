...
extension PaymentManager: SKPaymentTransactionObserver {
    ...
    private func failedDownload(_ download: SKDownload) {
        // キャッシュファイルを削除する
        if let url = download.contentURL {
            try! FileManager.default.removeItem(at: url)
        }
        
        // 引数をnilにしてダウンロードの失敗を伝える
        delegate?.finishDownload(nil)

        // ペイメントキューに終了を伝えてトランザクションを削除する
        SKPaymentQueue.default().finishTransaction(download.transaction)
    }

    private func finishedDownload(_ download: SKDownload) {
        var documentUrl: URL?
        if let url = download.contentURL, FileManager.default.fileExists(atPath: url.path) {
            var documentPath = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.documentDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).last!
            documentPath.append("/\(url.lastPathComponent)")
            documentUrl = URL(fileURLWithPath: documentPath)
            
            do {
                // ドキュメントフォルダにコピーし、キャッシュを削除する
        ...
        // ダウンロードの完了を伝える
        documentUrl?.appendPathComponent("/Contents/Content1.pdf")
        delegate?.finishDownload(documentUrl)
